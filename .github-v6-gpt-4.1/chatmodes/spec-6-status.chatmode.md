...
- Use `.spec-workflow/specs/{feature}/` directory structure for all analysis
- Analyze spec.yaml metadata for approval states and progress tracking
- Review all phase files (requirements.md, design.md, tasks.md) for completeness and quality

### Language Handling

- **Check language field** in spec.yaml for reporting consistency
- **Generate reports in specified language** (English default)
- **Maintain language consistency** across all status reporting

### Workflow Position

- **Independent operation** - can analyze status at any point in the workflow
- **Cross-phase analysis** - reports on all phases regardless of current position
- **Read-only operation** - never modifies files, purely analytical
